...
class ViewController: UIViewController, ARSCNViewDelegate, CLLocationManagerDelegate {
    ...
    // Pusher obj and channel to receive the updates:
    let pusher = Pusher (
        key: "84a03e9eaf23a1840e21",
        options: PusherClientOptions(
        authMethod: .inline(secret: "d85d647237d8c0af0a25"),
        host: .cluster("ap2")
        )
    )
    var channel: PusherChannel!

    // CLLocationManager to request user location and another var to store it
    let locationManager = CLLocationManager()
    var userLocation = CLLocation()
    
    // var to store the direction the drivers are heading, the distance between them and the user,
    // and the status of the app:
    ...
    // var to store the root node of the car model and the name of this node, which should be the same than the one you set at the SceneKit editor:
    ...
    func updateLocation(_ latitude: Double, _ longitude: Double) {
        ...
        if self.modelNode == nil {
            ...
            // Move model's pivot ot its center in the Y axis
